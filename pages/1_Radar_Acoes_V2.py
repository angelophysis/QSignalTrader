...
def _render_analysis(ticker):
    ...
    if rsi_d3 is not None or vol_rel is not None:
        ...
        c1, c2 = st.columns(2)
        with c1:
            rsi_note = ("RSI melhorou nos últimos 3 candles." if isinstance(rsi_d3, (int, float)) and rsi_d3 > 2
                        else "RSI perdeu força nos últimos 3 candles." if isinstance(rsi_d3, (int, float)) and rsi_d3 < -2
